...
public final class Window {
    ...
    public void showWindow(String title, Type type, int index) {
        isShowing = true;
        int frontType = type.value();
        Bundle bundle = new Bundle();
        bundle.putString(FloatWindow.NUMBER_INFO, title);
        if (index == 1) {
            bundle.putInt(FloatWindow.WINDOW_COLOR, ContextCompat.getColor(mContext,
                    R.color.red));
        } else {
...
//    public void showWindow(InCallModel number, Type type) {
//        isShowing = true;
//
//        int frontType = type.value();
//
